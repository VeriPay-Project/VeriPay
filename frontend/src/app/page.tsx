    ...
    const fetchStats = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:8000"}/stats/landing`,
          { credentials: "include" }
        )
      ...
      <div className="relative w-full max-w-6xl flex flex-col gap-20">
        ...
        <section className="grid grid-cols-3 gap-6 text-center motion-safe:animate-in motion-safe:fade-in duration-1000">
          <Metric
            ...
            value={<Counter target={stats.total_invoices} />}
          />

          <Metric
            title="Fraud signals detected"
            value={<Counter target={stats.fraud_signals} />}
          />

          <Metric
            title="Registered vendors"
            value={<Counter target={stats.total_vendors} />}
          />
        </section>
