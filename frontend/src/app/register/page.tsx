...
export default function RegisterPage() {
  ...
  const isStepValid = useCallback(
    ...
    [email, fullName, strength, confirmPassword, dobDate, securityQuestion, securityAnswer]
  )
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (Array.isArray(error.detail)) {
          setStatus(error.detail.map((e: { msg: string }) => e.msg).join(", "))
        } else {
              ...
              <div className="flex flex-col gap-2">
                <div className="flex gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <div
                      key={`str-${i}`}
                      className={`h-1 flex-1 rounded-full transition-all duration-300 ${i < props.strength ? strengthColors[i] : "bg-border"
                        }`}
                    />
                  ))}
                </div>
